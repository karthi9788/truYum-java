...
public class CartDaoSqlImpl {
	public List<MenuItem> getAllCartItems(long userId) throws CartEmptyException, ClassNotFoundException, SQLException {
		...
		Cart cart = new Cart((List) new ArrayList<MenuItem>(), 0);
		PreparedStatement preparedStatement = connection.prepareStatement(
				"select * from cart inner join menu_item on cart.menuitemid=menu_item.id where cart.userid=?;");
		...
		preparedStatement = connection.prepareStatement("select sum(price) from cart inner join menu_item on cart.menuItemId=menu_item.id where cart.userid=?;");
		preparedStatement.setLong(1, userId);
	...
	public void removeCartItem(long userId, long menuItemId) throws ClassNotFoundException, SQLException {
		Connection connection = ConnectionHandler.getConnection();
		PreparedStatement preparedStatement = connection.prepareStatement(
				"delete from cart where userId=? and menuItemId=?;");
		preparedStatement.setLong(1, userId);
		preparedStatement.setLong(2, menuItemId);
		preparedStatement.executeUpdate();
	}
